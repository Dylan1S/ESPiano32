...
void esp_ble_midi_on_bep_received(const uint8_t *data, uint16_t len)
{
    ...
    while (idx < len) {
        /* CHANGED THIS SECTION - previously always assumes first yte is timestamp, now accepts running status without a timestamp byte
        * Releasing many notes at once sends single midi packet which this was failing to parse which this change fixes.
        */
        if ((data[idx] & 0x80)) {
            //handle timestep byte
            ts_ms = (uint16_t)((ts_ms & 0x1F80) | (data[idx++] & 0x7F));
        }
        else if (running == 0 || s_sysex.active)
        {
