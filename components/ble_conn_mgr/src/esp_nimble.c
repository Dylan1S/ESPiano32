    ...
    switch (event->type) {
        ...
        case BLE_GAP_EVENT_NOTIFY_RX:
            ...
            if (!chr) {
                ...
                /*ESPiano32: tha ttr cache frees its buffer on next notif, while 
                * posted event still points at it (use after free under bursts). Give cache its own copy; event buffer is owned and freed by handler
                */
                uint8_t *cache_copy = malloc(conn_data.data_len);
                if(cache_copy) {
                    memcpy(cache_copy, conn_data.data, conn_data.data_len);  
                }
                rc = esp_ble_conn_on_gatts_attr_value_set(event->notify_rx.attr_handle, &chr->chr.uuid.u, cache_copy ? conn_data.data_len : 0, cache_copy);
